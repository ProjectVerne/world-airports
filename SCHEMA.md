...
Files are **sharded** by `iso_country` then by the first character of the key, so
no directory holds more than a few hundred files (GitHub's web UI truncates flat
directories at 1,000 entries):

```
data/airports/US/k/klax.json
data/airports/US/_/_us_00001.json
data/airports/DE/e/eddf.json
data/metros/_/lon.json
```
...
- The shard path is derived from the record (`iso_country`, first char of `key`)
  and is regenerated by tooling — contributors should not hand-place files.
